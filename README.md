...
- 月度结算：谁付给谁多少、每人实付 / 应担 / 差额、分类环形图（占比最大的 6 类，其余合并）
- 跨月余额：支出和转账进同一条流水，余额滚动累计。部分还款、多还、几个月一起还都不用特殊处理；转账可修改删除；「对账起点」之前的记录归档不计入
- 图标：每个分类一个线性图标，可在设置里给任意分类换图标（存在 config.json 的 categoryIcons 里）
- 电脑端：左侧月份栏（带各月总额，可收起，状态记在本机）+ 顶部常驻输入条（金额、备注、日期、付款人、全部分类一次到位，分摊方式收在次要一行）+ 明细表格 + 右侧结算与饼图；窄屏自动切回标签页布局
- 付款人默认是本机选的「我是谁」，所以两人共用一个 token 时，各自设备上都默认自己付款
- 设置：成员改名 / 添加，分类增删排序和图标，货币符号
