...
export function Sidebar({ months, totals, settled, month, config, meName, onPick, onCollapse, onSettings }: Props) {
      ...
      <nav className="side-months" aria-label="月份">
        ...
        {list.map((m) => (
          <button key={m} className={`side-month ${m === month ? 'on' : ''}`} aria-current={m === month ? 'true' : undefined}
            ...
            <span className="num">
              {totals[m] !== undefined ? fmt(totals[m], config.currency) : ''}
              {settled[m] && <span className="side-dot" title="已结清" />}
            </span>
          </button>
        ))}
      </nav>
